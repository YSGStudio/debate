...
import { listArchivedTeamDebates } from "@/lib/db/team-debates";
...
export async function GET() {
  ...
  const [classes, sessions, teamDebates] = await Promise.all([
    listArchivedClasses(auth.teacherId),
    listArchivedSessions(auth.teacherId),
    listArchivedTeamDebates(auth.teacherId),
  ]);

  return NextResponse.json({ classes, sessions, teamDebates });
}
