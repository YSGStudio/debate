...
export default function TeacherHome() {
  ...
  return (
    <div className="flex flex-col gap-8">
      <section>
        <h1 className="mb-4 text-xl font-bold">💬 내 토론</h1>
        {classes.length === 0 ? (
          <p className="text-gray-500">아직 토론이 없습니다. 아래에서 만들어 주세요.</p>
        ) : (
          <ul className="grid gap-3 sm:grid-cols-2">
            {classes.map((c) => (
              <li key={c.id} className="rounded-2xl border bg-white p-5">
                ...
                <ArchiveButton url={`/api/classes/${c.id}`} onDone={load} className="mt-3" />
              </li>
            ))}
          </ul>
        )}
      ...
      <section className="rounded-2xl border bg-white p-5">
        <h2 className="mb-3 font-bold">➕ 새 토론 만들기</h2>
        <form onSubmit={create} className="flex flex-wrap items-center gap-3">
          <input value={name} onChange={(e) => setName(e.target.value)} required placeholder="토론 이름 (예: 스마트폰 사용 토론)"
